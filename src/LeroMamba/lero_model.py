...
class GatedFeedForward(nn.Module):
    ...
    def forward(self, x: BatchedTree) -> BatchedTree:
        y = self.fc1(x)
...
class MambaBlock(nn.Module):
    def __init__(self, d_model, has_ff=True, d_state=16, expand=2):
        ...
        self.mamba = TreeMamba(d_model=d_model, d_state=d_state, expand=expand)
        if self.has_ff:
            self.norm2 = nn.LayerNorm(d_model)
            self.ff = GatedFeedForward(d_model, expand * d_model)

    def forward(self, batched_trees: BatchedTree):
        x = batched_trees.x
...
class LeroNet(torch.nn.Module):
    def __init__(self, input_feature_dim: int) -> None:
        ...
        self.cost_proj = torch.nn.Linear(self.dim, self.dim)
        self.output_act = nn.LeakyReLU()
        self.cost_est = nn.Linear(self.dim, 1)

    def forward(self, batched_trees: BatchedTree) -> torch.Tensor:
