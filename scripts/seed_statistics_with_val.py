...
val_pattern = re.compile(r'Validation ability: ([\d\.]+)%, pred time: ([\d\.]+), min time: ([\d\.]+), (\d+) timeouts:.*')
...
def read_output(path: str):
    ...
    val_abilities = np.zeros(100, dtype=np.float32)
    val_pred_times = np.zeros(100, dtype=np.float32)
    val_nums_timeout = np.zeros(100, np.float32)
    ...
    for line in lines:
        ...
        if result is not None:
            ...
            train_losses[epoch] = train_loss
            continue
        result = val_pattern.match(line)
        if result is not None:
            ability, pred, _, num_timeout = result.groups()
            val_abilities[epoch] = ability
            val_pred_times[epoch] = pred
            val_nums_timeout[epoch] = num_timeout
            continue
    ...
    print(epoch)
    return train_losses[:epoch + 1], abilities[:epoch + 1], pred_times[:epoch + 1], nums_timeout[:epoch + 1], val_abilities[:epoch + 1], val_pred_times[:epoch + 1], val_nums_timeout[:epoch + 1], 
...
def read_outputs(pattern: str, seeds: list[int]):
    ...
    ret_val_abilities = []
    ret_val_pred_times = []
    ret_val_nums_timeout = []
    for seed in seeds:
        path = pattern.format(seed)
        train_losses, abilities, pred_times, nums_timeout, val_abilities, val_pred_times, val_nums_timeouts  = read_output(path)
        ...
        ret_val_abilities.append(val_abilities)
        ret_val_pred_times.append(val_pred_times)
        ret_val_nums_timeout.append(val_nums_timeouts)
    return np.stack(ret_train_losses), np.stack(ret_abilities), np.stack(ret_pred_times), np.stack(ret_nums_timeout), np.stack(ret_val_abilities), np.stack(ret_val_pred_times), np.stack(ret_val_nums_timeout)

def plot_dir(base_dir: str, num_seeds: int):
    train_loss, abilities, _, _, val_abilities, _, _ = read_outputs(os.path.join(base_dir, 'output_{}.txt'), range(num_seeds))
    ...
    selected_abilities = []
    for i in range(val_abilities.shape[0]):
        max_index = np.argmax(val_abilities[i, :])
        selected_ability = abilities[i, max_index]
    ...
    fig, ax = plt.subplots(figsize=(num_seeds, 6), dpi=600)
    ax.bar(np.arange(num_seeds), selected_abilities, yerr=abilities_std, capsize=5,
        edgecolor='black', facecolor='none', linewidth=1.5, ecolor='black', error_kw={'elinewidth': 2})

    ax.set_title('Comparison of Ability Across Random Seeds')
    ax.set_xlabel('Random Seed')
    ...
    # Save the figure
    fig.savefig(os.path.join(base_dir, 'random_seeds_max_val_test_abilities.png'))
    
    print(f'Done! Ability={np.mean(selected_abilities)}')
