...
def main(args):
    ...
    for model, workload, method, seed in itertools.product(args.models, args.workloads, args.methods, args.seeds):
        ...
        command = f'./src/{model_dirname}/train.py --dataset {dataset_dirname}/{workload}/{method_dirname} --test {dataset_dirname}/{workload_dirname}/{method_dirname} --valset=empty.json --seed {seed} --epoch {epoch} --batch_size {batch_size} --lr={learning_rate} --logdir=expriments/{model}/{workload}/{method}/{seed}'
        print(command)
        os.system(command)
